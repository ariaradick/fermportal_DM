...
lambda_values = np.genfromtxt("../relic_abundance/uR_model/uR_lambdas_grid.csv")
...
def create_script_file(proc, hel1, hel2, pdf_resc='1e-4', fact_scale=None):
    ...
    if proc == 'mumu':
        if fact_scale == 'dyn':
            filename = "madgraph_run_files_uR/run_mu{}mu{}_to_phiphibar_chichiuubar_resc{}_fact_dyn.txt".format(hel1, hel2, pdf_resc)
            generate_line = "generate mu+{{{}}} mu-{{{}}} > phi phibar > u u~ chi chi~ QED<=3\n".format(hel1, hel2)
            run_name = "fermportal_uR_mu{0}mu{1}_to_phiphibar_chichiuubar_resc{2}_fact_dyn".format(hel1, hel2, pdf_resc)

        else: # the default (factorization scale = beam energy )
            filename = "madgraph_run_files_uR/run_mu{}mu{}_to_phiphibar_chichiuubar_resc{}.txt".format(hel1, hel2, pdf_resc)
            generate_line = "generate mu+{{{}}} mu-{{{}}} > phi phibar > u u~ chi chi~ QED<=3\n".format(hel1, hel2)
            run_name = "fermportal_uR_mu{0}mu{1}_to_phiphibar_chichiuubar_resc{}".format(hel1, hel2, pdf_resc)

    elif proc == 'vv':
        filename = 'madgraph_run_files_uR/run_vv_to_phiphibar_chichiuubar.txt'
        generate_line = 'generate vxp vxm > phi phibar > u u~ chi chi~ QED<=3\n'
        run_name = 'fermportal_uR_vv_to_phiphibar_chichiuubar'

    elif proc == 'aa':
        filename = 'madgraph_run_files_uR/run_aa_to_phiphibar_chichiuubar.txt'
        generate_line = 'generate a a > phi phibar > u u~ chi chi~ QED<=3\n' 
        run_name = 'fermportal_uR_aa_to_phiphibar_chichiuubar'

    elif proc == 'ww':
        filename = 'madgraph_run_files_uR/run_ww_to_phiphibar_chichimumu.txt'
        generate_line = 'generate w+ w- > phi phibar > u u~ chi chi~ QED<=3\n'
        run_name = 'fermportal_uR_ww_to_phiphibar_chichiuubar'
    ...
    with open(filename, 'w') as file:
        file.write('import model fermportal_uR\n')
