...
function mumu_script(mphis, lam, mchi, hels, lhaids, lpp_pairs; 
    ...
    for hi in eachindex(hels)
        ...
        res *= """import model fermportal_QL
            generate mu+{$hp} mu-{$hm} > scau scaub QED<=3
            add process mu+{$hp} mu-{$hm} > scad scadb
            output $hdir
            """
...
function vv_script(mphis, lam, mchi; sde_strategy=2, nevents=10000, dir=DIR_VVφφ)
    ...
    define vxp = w+ z a
    define vxm = w- z a
    generate vxp vxm > scau scaub QED<=3 LFDM=0
    add process vxp vxm > scau scadb QED<=3 LFDM=0
    add process vxp vxm > scad scaub QED<=3 LFDM=0
    add process vxp vxm > scad scadb QED<=3 LFDM=0 
    output $dir
    """
